...
test("GIVEN ResourceLogsView WHEN filtering changes AND we are not on the first page THEN we are sent back to the first page", async () => {
  const { component, apiHelper } = setup();
  ...
  await act(async () => {
    await userEvent.click(screen.getByLabelText("Action TypeFilterInput"));
  });
  await act(async () => {
    await userEvent.click(screen.getByRole("option", { name: "store" }));
  });
  ...
  expect(apiHelper.pendingRequests[1].url).not.toMatch(/(&start=|&end=)/);
  expect(apiHelper.pendingRequests[1].url).toMatch(/(&filter.action=store)/);
});
