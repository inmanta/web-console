...
describe("LatestCompileReportRow", () => {
  it("shows the trigger and message, duration and a 'View report' link for a successful compile", () => {
    ...
    const link = screen.getByRole("link", {
      name: `${words("dashboard.compileReports.viewReport")} >`,
    });
  ...
  it("shows a danger 'View report' link for a failed compile", () => {
    ...
    expect(
      screen.getByRole("link", { name: `${words("dashboard.compileReports.viewReport")} >` })
    ).toBeInTheDocument();
  });
  ...
  it("shows 'Running…' and 'started X ago' with a 'View progress' link while in progress", () => {
    ...
    expect(screen.getByText(words("dashboard.compileReports.running"))).toBeVisible();
    expect(
      screen.getByText(words("dashboard.compileReports.startedAgo"), { exact: false })
    ).toBeVisible();

    const link = screen.getByRole("link", {
      name: `${words("dashboard.compileReports.viewProgress")} >`,
    });
  ...
  it("shows 'Queued' without a 'started X ago' line for a queued compile", () => {
    ...
    expect(screen.getByText(words("dashboard.compileReports.queued"))).toBeVisible();
    expect(
      screen.queryByText(words("dashboard.compileReports.startedAgo"), { exact: false })
    ).not.toBeInTheDocument();

    expect(
      screen.getByRole("link", { name: `${words("dashboard.compileReports.viewProgress")} >` })
    ).toBeInTheDocument();
  });
