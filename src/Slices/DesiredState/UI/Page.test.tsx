...
test("GIVEN DesiredState WHEN filtering changes AND we are not on the first page THEN we are sent back to the first page", async () => {
  const { component, apiHelper } = setup();
  ...
  await act(async () => {
    apiHelper.resolve(
      Either.right({
        data: DesiredStateVersions.response.data,
        metadata: {
          total: 23,
          before: 0,
          after: 3,
        ...
        links: {
          self: DesiredStateVersions.response.links.self,
  ...
  await act(async () => {
    await userEvent.click(screen.getByLabelText("StatusFilterInput"));
  });
  await act(async () => {
  ...
  expect(apiHelper.pendingRequests[1].url).toMatch(/(&filter.status=active)/);
});
