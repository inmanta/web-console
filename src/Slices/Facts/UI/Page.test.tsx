...
test("GIVEN FactsView WHEN filtering changes AND we are not on the first page THEN we are sent back to the first page", async () => {
  const { component, apiHelper } = setup();
  ...
  await act(async () => {
    await userEvent.click(screen.getAllByLabelText("submit search")[0]);
  });
