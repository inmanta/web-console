...
function setup() {
  ...
  const queryResolver = new QueryResolverImpl(new DynamicQueryManagerResolverImpl([queryManager]));
  const fileFetcher = new FileFetcherImpl(apiHelper);

  fileFetcher.setEnvironment("env");
  const component = (
    <DependencyProvider dependencies={{ ...dependencies, fileFetcher, queryResolver }}>
