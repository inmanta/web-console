...
describe("LatestCompileReportsPanel", () => {
  ...
  it("shows the title and a link to the full compile reports page", async () => {
    ...
    expect(await screen.findByText(words("dashboardV2.compileReports.title"))).toBeVisible();

    const link = screen.getByRole("link", { name: words("dashboardV2.compileReports.viewAll") });
  ...
  it("shows an empty message when there are no compile reports", async () => {
    ...
    expect(await screen.findByText(words("dashboardV2.compileReports.empty"))).toBeVisible();
  });
  ...
  it("renders one row per compile report returned by the query", async () => {
    ...
    expect(screen.queryByText(words("dashboardV2.compileReports.empty"))).not.toBeInTheDocument();
  });
});
