...
describe("OrchestrationEngineCard", () => {
  ...
  it("shows the title, subtitle and the Compiles/Failed/Avg compile/Avg waiting stats", async () => {
    ...
    expect(screen.getByText(words("dashboardV2.orchestrationEngine.title"))).toBeVisible();
    expect(screen.getByText(words("dashboardV2.orchestrationEngine.subtitle")(7))).toBeVisible();

    await expectStat("compiles", "142", words("dashboardV2.orchestrationEngine.stats.compiles"));
    await expectStat("failed", "3", words("dashboardV2.orchestrationEngine.stats.failed"));
    ...
    await expectStat(
      "avg-compile",
      "13.22s",
      words("dashboardV2.orchestrationEngine.stats.avgCompile")
    );
    await expectStat(
      "avg-waiting",
      "5.28s",
      words("dashboardV2.orchestrationEngine.stats.avgWaiting")
    );
  });
  ...
  it("defaults to the Compile rate tab, and switches the chart title when another tab is selected", async () => {
    ...
    expect(
      await screen.findByText(words("dashboardV2.orchestrationEngine.chart.rate.title"))
    ).toBeVisible();

    await userEvent.click(
      screen.getByRole("button", { name: words("dashboardV2.orchestrationEngine.tabs.time") })
    );

    expect(
      screen.getByText(words("dashboardV2.orchestrationEngine.chart.time.title"))
    ).toBeVisible();
    expect(
      screen.queryByText(words("dashboardV2.orchestrationEngine.chart.rate.title"))
    ).not.toBeInTheDocument();

    await userEvent.click(
      screen.getByRole("button", { name: words("dashboardV2.orchestrationEngine.tabs.waiting") })
    );

    expect(
      screen.getByText(words("dashboardV2.orchestrationEngine.chart.waiting.title"))
    ).toBeVisible();
  });
  ...
  it("defaults to a 7-day range, and switches to 14/30 days when picked from the range dropdown", async () => {
    ...
    expect(
      screen.getByRole("button", { name: words("dashboardV2.orchestrationEngine.range.last7Days") })
    ).toBeVisible();
    expect(screen.getByText(words("dashboardV2.orchestrationEngine.subtitle")(7))).toBeVisible();
    ...
    await userEvent.click(
      screen.getByRole("menuitem", {
        name: words("dashboardV2.orchestrationEngine.range.last14Days"),
      })
    );
    ...
    expect(screen.getByText(words("dashboardV2.orchestrationEngine.subtitle")(14))).toBeVisible();
  });
  ...
  it("refetches the metrics when the refresh button is clicked", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: words("dashboardV2.orchestrationEngine.refresh") })
    );
