...
const RANGE_OPTIONS: { value: RangeDays; label: string }[] = [
  { value: 7, label: words("dashboardV2.orchestrationEngine.range.last7Days") },
  { value: 14, label: words("dashboardV2.orchestrationEngine.range.last14Days") },
  { value: 30, label: words("dashboardV2.orchestrationEngine.range.last30Days") },
];
...
const TABS: TabConfig[] = [
  {
    key: "rate",
    label: words("dashboardV2.orchestrationEngine.tabs.rate"),
    chartTitle: words("dashboardV2.orchestrationEngine.chart.rate.title"),
    color: chart_color_orange_300.var,
    formatValue: (value) => value.toFixed(2),
  },
  {
    key: "time",
    label: words("dashboardV2.orchestrationEngine.tabs.time"),
    chartTitle: words("dashboardV2.orchestrationEngine.chart.time.title"),
    color: chart_color_blue_300.var,
    formatValue: (value) => `${value.toFixed(2)}s`,
  },
  {
    key: "waiting",
    label: words("dashboardV2.orchestrationEngine.tabs.waiting"),
    chartTitle: words("dashboardV2.orchestrationEngine.chart.waiting.title"),
      ...
      >
        ...
        <FlexItem>
          <Button variant="secondary" icon={<SyncIcon />} onClick={handleRefresh}>
            {words("dashboardV2.orchestrationEngine.refresh")}
          </Button>
        </FlexItem>
      </Flex>
      <FlexItem>
        <Card>
          <CardHeader
            actions={{
              hasNoOffset: true,
              actions: (
                <ToggleGroup aria-label={words("dashboardV2.orchestrationEngine.title")}>
                  {TABS.map((tabConfig) => (
                    <ToggleGroupItem
                      key={tabConfig.key}
                ...
                <Flex direction={{ default: "column" }} spaceItems={{ default: "spaceItemsNone" }}>
                  <FlexItem>
                    <Content component="h3">
                      {words("dashboardV2.orchestrationEngine.title")}
                    </Content>
                  </FlexItem>
                  <FlexItem>
                    <Content component="small">
                      {words("dashboardV2.orchestrationEngine.subtitle")(days)}
                    </Content>
                  </FlexItem>
                </Flex>
              </Flex>
          ...
          <CardBody>
            <Flex spaceItems={{ default: "spaceItemsXl" }} columnGap={{ default: "columnGapXl" }}>
              <StatItem
                ...
                label={words("dashboardV2.orchestrationEngine.stats.compiles")}
              />
              <StatItem
                testId="failed"
                value={(Number(failedCompiles?.metadata.total) || 0).toLocaleString()}
                label={words("dashboardV2.orchestrationEngine.stats.failed")}
                isDanger
              />
              <StatItem
                testId="avg-compile"
                value={formatSeconds(avgCompileSeconds)}
                label={words("dashboardV2.orchestrationEngine.stats.avgCompile")}
              />
              <StatItem
                testId="avg-waiting"
                value={formatSeconds(avgWaitingSeconds)}
                label={words("dashboardV2.orchestrationEngine.stats.avgWaiting")}
              />
            </Flex>
          </CardBody>
          <CardBody>
            <Flex direction={{ default: "column" }} spaceItems={{ default: "spaceItemsSm" }}>
              ...
              <FlexItem>
                <TrendChart
                  ...
                  xStartLabel={words("dashboardV2.orchestrationEngine.chart.xStart")(days)}
                  xEndLabel={words("dashboardV2.orchestrationEngine.chart.xEnd")}
