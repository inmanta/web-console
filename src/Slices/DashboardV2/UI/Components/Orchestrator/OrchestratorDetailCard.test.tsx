...
describe("OrchestratorDetailCard", () => {
  ...
  it("shows the title, product details and extensions, with a link to the full Status page", async () => {
    ...
    expect(screen.getByText(words("dashboardV2.orchestrator.title"))).toBeVisible();
    ...
    const link = screen.getByRole("link", {
      name: `${words("dashboardV2.orchestrator.viewFullStatus")} >`,
    });
  ...
  it("shows an Operational badge and a checkmark when the license is valid and server/database/scheduler are all ok", async () => {
    ...
    expect(
      await screen.findByText(words("dashboardV2.environmentHealth.operational"))
    ).toBeVisible();
    expect(
      screen.queryByText(words("dashboardV2.environmentHealth.status.attention"))
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(words("dashboardV2.environmentHealth.status.danger"))
    ).not.toBeInTheDocument();
  });
  ...
  it("shows an Attention badge and a warning triangle when the license has expired but server/database/scheduler are ok", async () => {
    ...
    expect(screen.getByText(words("dashboardV2.environmentHealth.status.attention"))).toBeVisible();
    expect(
      screen.queryByText(words("dashboardV2.environmentHealth.operational"))
    ).not.toBeInTheDocument();
  });
  ...
  it("shows a Danger badge and an error icon when the database is not connected, even with a valid license", async () => {
    ...
    expect(
      await screen.findByText(words("dashboardV2.environmentHealth.status.danger"))
    ).toBeVisible();
    expect(
      screen.queryByText(words("dashboardV2.environmentHealth.operational"))
    ).not.toBeInTheDocument();
