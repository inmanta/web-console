...
const formatLicenseValue = (serverStatus: ServerStatus): string => {
  ...
  return expiry && isValid !== undefined
    ? words("dashboardV2.orchestrator.licenseSummary")(
        isValid,
...
const SEVERITY_LABEL_TEXT: Record<Severity, string> = {
  success: words("dashboardV2.environmentHealth.operational"),
  warning: words("dashboardV2.environmentHealth.status.attention"),
    ...
    <Card isFullHeight>
        ...
        <CardTitle>
          ...
          >
            ...
            <Flex direction={{ default: "column" }} spaceItems={{ default: "spaceItemsNone" }}>
              <FlexItem>
                <Content component="h3">{words("dashboardV2.orchestrator.title")}</Content>
              </FlexItem>
              <FlexItem>
      ...
      <CardBody>
        <Flex direction={{ default: "column" }} spaceItems={{ default: "spaceItemsSm" }}>
          <DetailRow
            label={words("dashboardV2.orchestrator.edition")}
            value={serverStatus?.edition ?? "—"}
          />
          <DetailRow
            label={words("dashboardV2.orchestrator.version")}
            value={serverStatus?.version ?? "—"}
          />
          <DetailRow
            label={words("dashboardV2.orchestrator.license")}
            value={serverStatus ? formatLicenseValue(serverStatus) : "—"}
          />
          <DetailRow
            label={words("dashboardV2.orchestrator.pythonVersion")}
            value={serverStatus?.python_version ?? "—"}
          />
          <DetailRow
            label={words("dashboardV2.orchestrator.postgresqlVersion")}
      ...
      <CardBody>
        <Flex direction={{ default: "column" }} spaceItems={{ default: "spaceItemsSm" }}>
          <FlexItem>
            <Content component="small">{words("dashboardV2.orchestrator.extensionsLabel")}</Content>
          </FlexItem>
              ...
              >
                ...
                <FlexItem>
                  <Label isCompact variant="outline">
                    {words("dashboardV2.orchestrator.extensionTag")}
                  </Label>
                </FlexItem>
              </Flex>
              <FlexItem>
      ...
      <CardFooter>
        <Link to={statusUrl}>
          <Button variant="link" isInline>
            {words("dashboardV2.orchestrator.viewFullStatus")} &gt;
          </Button>
        </Link>
      </CardFooter>
    </Card>
