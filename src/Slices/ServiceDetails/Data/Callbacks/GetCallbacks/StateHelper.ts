...
type ApiData = RemoteData.Type<Query.Error<"GetCallbacks">, Query.ApiResponse<"GetCallbacks">>;
...
export function CallbacksStateHelper(store: Store) {
  function sanitize(data: ApiData, service_entity: string): Data {
    ...
    const serviceCallbacks = allCallbacks.filter((cb) => cb.service_entity === service_entity);
  ...
  return PrimaryStateHelperWithEnv<"GetCallbacks">(
    ...
    (state, { service_entity }, environment) => state.callbacks.byEnv[environment]?.[service_entity]
  );
}
