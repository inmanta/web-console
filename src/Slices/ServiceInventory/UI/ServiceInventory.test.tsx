...
test("GIVEN ServiceInventory WHEN filtering changes AND we are not on the first page THEN we are sent back to the first page", async () => {
  const { component, apiHelper } = setup({ ...Service.a, owner: "owner" });
  ...
  await act(async () => {
    await userEvent.click(screen.getByLabelText("StateFilterInput"));
  });
  await act(async () => {
  ...
  expect(apiHelper.pendingRequests[1].url).not.toMatch(/(&start=|&end=)/);
  expect(apiHelper.pendingRequests[1].url).toMatch(/(&filter.state=creating)/);
});
