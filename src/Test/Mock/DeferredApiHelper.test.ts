...
test("GIVEN BaseApiHelper POST WHEN response contains large integers THEN integers are converted to bigints", async () => {
  const apiHelper = new DeferredApiHelper();

  //POST
  apiHelper.post<{ foo: bigint }, { foo: bigint }>("", "", {
    foo: 9223372036854775807n,
  });
  apiHelper.pendingRequests;
  ...
  apiHelper.resolveRequest(apiHelper.pendingRequests[0], {});

  //POST without Response
  apiHelper.postWithoutResponse<{ foo: bigint }>("", "", {
    foo: 9223372036854775807n,
  });
  apiHelper.pendingRequests;
  expect(apiHelper.pendingRequests[0]).toEqual({
  ...
  //POST without Response and Environment
  apiHelper.postWithoutResponseAndEnvironment<{ foo: bigint }>("", {
    foo: 9223372036854775807n,
  });
  apiHelper.pendingRequests;
  expect(apiHelper.pendingRequests[0]).toEqual({
...
test("GIVEN BaseApiHelper PUT WHEN response contains large integers THEN integers are converted to bigints", async () => {
  const apiHelper = new DeferredApiHelper();
  apiHelper.putWithoutEnvironment<{ foo: bigint }, { foo: bigint }>("", {
    foo: 9223372036854775807n,
  });
  apiHelper.pendingRequests;
  expect(apiHelper.pendingRequests[0]).toEqual({
...
test("GIVEN DeferredApiHelper PATCH reqiest WHEN request body contains large integers THEN integers are converted to bigints", async () => {
  const apiHelper = new DeferredApiHelper();
  apiHelper.patch<{ foo: bigint }>("", "", {
    foo: 9223372036854775807n,
  });
  apiHelper.pendingRequests;
  expect(apiHelper.pendingRequests[0]).toEqual({
