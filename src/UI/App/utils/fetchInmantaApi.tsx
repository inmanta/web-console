...
export async function fetchInmantaApi(
  ...
  try {
    ...
    if (!requestParams.method || requestParams.method === "GET") {
      json = await doFetch(
        fullEndpointPath,
        requestParams.isEnvironmentIdRequired,
        requestParams.environmentId,
        requestParams.keycloak
      );
    } else {
      json = await postWithFetchApi(
        fullEndpointPath,
        requestParams.environmentId,
...
async function doFetch(
  ...
  if (isEnvironmentIdRequired && environmentId) {
    ...
    result = await fetch(`${urlEndpoint}`, {
      headers: extendHeadersWithAuth({}, keycloak),
    });
  }
  if (result) {
    if (!result.ok) {
      ...
      throw Error(
        `The following error occured while fetching data: ${result.status} ${result.statusText} ${errorMessage}`
      );
    }
    return result.json();
  }
...
async function postWithFetchApi(
  urlEndpoint,
  environmentId,
  method = "POST",
  data = "",
  ...
  if (result) {
    if (!result.ok) {
      ...
      throw Error(
        `The following error occured while communicating with the server: ${
          result.status
        } ${result.statusText} ${
