...
describe("EnvironmentHandler", () => {
  ...
  test("EnvironmentHandler updates environment correctly", async () => {
    ...
    const env = Environment.previewFilterable[0];
  ...
  test("EnvironmentHandler determines selected environment correctly", async () => {
    ...
    history.push(`?env=${Environment.previewFilterable[0].id}`);
    expect(result.current.determineSelected([], history.location.search)).toBeUndefined();

    expect(
      result.current.determineSelected(Environment.previewFilterable, history.location.search)
    ).toEqual(Environment.previewFilterable[0]);

    result.current.set(history.push, history.location, Environment.previewFilterable[1].id);
    expect(
      result.current.determineSelected(Environment.previewFilterable, history.location.search)
    ).toEqual(Environment.previewFilterable[1]);
  });
  ...
  test("Given the environmentModifier When the server compile setting is requested Then returns the correct value", async () => {
    ...
    await act(async () => {
      result.current.set(history.push, history.location, Environment.previewFilterable[0].id);
    });
