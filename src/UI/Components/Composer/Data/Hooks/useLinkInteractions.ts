...
import { canRemoveLink, getRelationInfo } from "../Helpers/relationsHelpers";
...
export const useLinkInteractions = ({
    ...
    useEffect(() => {
        ...
        const handleGraphRemove = (cell: dia.Cell) => {
            if (cell instanceof dia.Link) {
                ...
                if (!graph || !canRemoveLink(sourceShape, targetShape, graph, relationsDictionary, serviceCatalog)) {
                ...
                if (removedShape.entityType === "relation" && !initialShapeInfoRef.current.has(removedShapeId)) {
    ...
    }, [paper, graph, canvasHandlers, setCanvasState, initialShapeInfoRef, relationsDictionary, serviceCatalog]);
