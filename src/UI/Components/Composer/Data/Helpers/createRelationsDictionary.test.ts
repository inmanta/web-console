...
const createServiceModel = (name: string, overrides: Partial<ServiceModel> = {}): ServiceModel => ({
  ...
  lifecycle: {
    ...
    states: [{ name: "active", deleted: false, export_resources: false, purge_resources: false }],
    transfers: [],
  },
  attributes: [],
  config: {},
...
describe("createRelationsDictionary", () => {
  ...
  it("should skip read-only embedded entities and their relations", () => {
    const catalog: ServiceModel[] = [
      createServiceModel("ServiceA", {
        embedded_entities: [
          createEmbeddedEntity("Details", "Details", {
            modifier: "r",
            lower_limit: 1,
            upper_limit: 1,
          }),
      ...
      createServiceModel("ServiceA", {
        embedded_entities: [
          createEmbeddedEntity("Endpoints", "Endpoint", {
            inter_service_relations: [
              createInterServiceRelation("ReadonlyRelation", "Other", { modifier: "r" }),
            ],
          }),
        ],
      }),
