...
it.each`
  ...
  ${{ updated: [{ operator: "from", date: new Date("2022-01-30T23:30:00.000Z") }] }} | ${{ name: "updated", order: "desc" }} | ${"20"}  | ${""}                                               | ${"/api/v2/parameters?limit=20&sort=updated.desc&filter.updated=ge%3A2022-01-30%2B23%3A30%3A00"}
...
`(
  ...
  ({ filter, pageSize, sort, url, currentPage }) => {
    const query: GetParametersParams = {
