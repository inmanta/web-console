...
import { getPaginationHandlers } from "@/Data/Queries/Helpers/Pagination/getPaginationHandlers";
...
export const useGetResourceHistory = (params: GetResourceHistoryParams): GetResourceHistory => {
  ...
  return {
    useOneTime: (): UseQueryResult<ResourceHistoryResponse, Error> =>
      useQuery({
        queryKey: ["get_resource_history-one_time", id, pageSize, sort, currentPage, env],
        queryFn: () => get(url),
        select: (data) => ({
          ...data,
