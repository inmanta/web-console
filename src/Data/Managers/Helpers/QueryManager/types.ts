...
type GetDependenciesWith<Kind extends Query.Kind, WithEnv> = WithEnv extends true
  ? (query: Query.SubQuery<Kind>, environment: string) => (string | number | boolean | undefined)[]
  : (query: Query.SubQuery<Kind>) => (string | number | boolean | undefined)[];

export type GetDependencies<Kind extends Query.Kind> = GetDependenciesWith<Kind, false>;
export type GetDependenciesWithEnv<Kind extends Query.Kind> = GetDependenciesWith<Kind, true>;
...
export type GetUniqueWithEnv<Kind extends Query.Kind> = GetUniqueWith<Kind, true>;
