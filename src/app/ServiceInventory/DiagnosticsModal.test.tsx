...
describe("getValidationFailureMessage function", () => {
  it("Should find the correct compile report when there are multiple in the logs", async () => {
        ...
        data: [
          {
            ...
            events: [
              {
                id_compile_report: "a",
                timestamp: "2020-09-08T18:06:04.264697",
              },
              {
                id_compile_report: "b",
                timestamp: "2020-09-08T18:06:09.130214",
              },
            ],
          },
        ],
    ...
    fetchMock.once(
      JSON.stringify({
        report: {
          reports: [{ returncode: 1, errstream: "Something happened" }],
          compile_data: { type: "ValidationError", message: errorMessage },
        },
      })
    );
    ...
    expect(JSON.parse(errorWithTrace.errorMessage).message).toEqual(
      errorMessage
    );
  });
