...
describe("Instance Modal ", () => {
  ...
  it.each`
    ...
    ${"DELETE"} | ${"Delete"}
  `(
    "Given buttonType $buttonType, should render button with text $text",
    ({ buttonType, text }) => {
      const wrapper = mount(
        <InstanceModal buttonType={buttonType} serviceName="test_service" />
      );
  ...
  it("Prepares delete instance form", () => {
    ...
    expect(modalHeader.text()).toContain(
      "delete instance instance1 of service entity test_service"
    );
  });
  ...
  describe("Edit form", () => {
      ...
      const wrapper = mount(
        ...
        >
          <InstanceModal
            buttonType={ButtonType.edit}
            serviceName="test_service"
            instance={attributeServiceInstanceModel}
          />
        </InventoryContext.Provider>
      );
      const button = wrapper.find("button");
    ...
    it("Provides correct default values when there are candidate attributes", () => {
      attributeServiceInstanceModel.candidate_attributes = {
        name: "candidate",
      };
      const wrapper = mount(
        <InventoryContext.Provider
