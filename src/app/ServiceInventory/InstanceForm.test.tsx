...
describe('Instance Form component', () => {
  ...
  describe('Update form', () => {
    it('Calls patch on instance url when submitted', () => {
      ...
      const wrapper = mount(<InstanceForm attributeModels={attributes} requestParams={requestParams} originalAttributes={originalAttributes} />);
      const button = wrapper.find(".pf-c-button.pf-m-primary");
      button.simulate('click');
      expect(fetchMock.mock.calls).toHaveLength(1);
    ...
    it('Shows info message when no attributes can be updated', () => {
      const wrapper = mount(<InstanceForm attributeModels={[]} requestParams={requestParams} originalAttributes={{}} />);
