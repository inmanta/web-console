...
describe('Breadcrumbs', () => {
  ...
  `('Should render $numberOfItems breadcrumb items for $entry', ({entry, numberOfItems}) => {
    const wrapper = mount(<MemoryRouter initialEntries={[entry]}><PageBreadcrumb /></MemoryRouter>);
  ...
  `('The current element shouldn\'t be clickable',({entry, activeElementIndex})=> {
    const wrapper = mount(<MemoryRouter initialEntries={[entry]}><PageBreadcrumb /></MemoryRouter>);
    const breadcrumbItems = wrapper.find('.pf-c-breadcrumb__item');
    expect(breadcrumbItems.at(activeElementIndex).hasClass('active')).toBeFalsy();
  });
