...
describe("Backend data fetching function", () => {
  ...
  const defaultRequestParams: IRequestParams = {
    ...
    setErrorMessage: dummyErrorSetter,
  };
  beforeEach(() => {
  ...
  describe("On failed request", () => {
    it("Should set the error message correctly for http errors", async () => {
      ...
      let errorMessage;
      const setErrorMessage: React.Dispatch<string> = (message) => {
  ...
  it("Should set content type and data on POST calls", async () => {
    ...
    fetchMock.mockResponse(
      JSON.stringify({ data: { ...data, id: "instanceId1" } })
    );
  ...
  it("Should handle errors on server state modifying calls", async () => {
    fetchMock.mockResponse(JSON.stringify({ message: "Invalid request" }), {
      status: 400,
      statusText: "Bad Request",
    });
  ...
  describe("When keycloak is enabled", () => {
    const requestParamsWithKeycloak = {
      ...defaultRequestParams,
      keycloak: {
        token: "JWT token",
        clearToken: () => {
    ...
    it("Should add authorization header to GET requests without environment id", async () => {
      await fetchInmantaApi({
        ...requestParamsWithKeycloak,
        isEnvironmentIdRequired: false,
      });
      expect(fetchMock.mock.calls).toHaveLength(1);
      expect(fetchMock.mock.calls[0][1].headers).toEqual({
    ...
    it("Should handle authorization errors on POST requests", async () => {
      fetchMock.mockResponse(
        JSON.stringify({
          message: "Access to protected resource is forbidden",
        }),
