...
async function getDevVersions() {
  const getQueryString = (cursor) => `{repository(owner:"inmanta",name:"web-console"){packages(first:1, names:"web-console"){nodes{packageType,name,id,versions(last:50, ${cursor} orderBy:{field:CREATED_AT, direction:DESC}){nodes{id,version,preRelease,files(first:1){nodes{name, updatedAt}}}, pageInfo{hasNextPage, hasPreviousPage,startCursor}}}}}}`;
  let queryString = getQueryString('');
  let moreRows = true;
  while (moreRows) {
    const queryResults = await fetch("https://api.github.com/graphql", {
    ...
    const webConsole = queryResults.data.repository.packages.nodes.find((package) => package.name === "web-console");
    ...
    if (devVersions.length == 0 && webConsole.versions.pageInfo.hasPreviousPage && webConsole.versions.pageInfo.startCursor) {
      queryString = getQueryString(`before: "${webConsole.versions.pageInfo.startCursor}",`);
      console.log(`No dev versions found on this page of the results, fetching the previous page from ${webConsole.versions.pageInfo.startCursor}`);
    } else {
